...
namespace DeNelle.Editor
{
    public static class DataRegression
    {
        public static void RunAll()
        {
            ...
            // --- WO-1743 google play store layout: the Play storefront's product rows were HAND-PLACED by fraction anchors (top .90, pitch .095, height .082) in a modal body that measures ~542 REFERENCE px at the owner's 2670x1200 Seeker -- a ~51px pitch carrying ~44px rows, which UiKitMinTouchGuard then grew to the 112px touch floor SYMMETRICALLY ABOUT EACH ROW'S CENTRE, spilling every row ~34px into both neighbours (logs/device/store-listing.png: frames drawn over frames, the subtitle half-covered, CLOSE on top of a product row). With 18 storeVisible packs the loop also ran to y1 = -0.715, straight through the fixed Restore/Deletion bands -- which is why RESTORE PURCHASES appears interleaved seven rows down. The kit had already named this exact pattern at ElarionUiKitObsidian.cs:574-580. Pins the fix: a kit FIT-OR-SCROLL zone with rows sized by explicit sizeDelta at the floor, bands disjoint, and a RED FIXTURE that rebuilds the old fraction column and fails the suite if it does NOT overlap. ---
