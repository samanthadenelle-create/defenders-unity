...
namespace DeNelle.Village
{
    ...
    public sealed class WaveManager : MonoBehaviour
    {
        ...
        // ── WO-1773: the EFFECTIVE concurrency cap ────────────────────────────────────────
        // _maxSimultaneousEnemies is the AUTHORED (serialized) value and stays the only thing a
        // scene carries. MaxSimultaneous is what every read site below uses: the authored value
        // with the remote-tunable wave.maxSimultaneousPct percent folded in.
        //
        // WHY CACHED PER WAVE AND NOT READ PER USE. RemoteTunables.Int costs a spec lookup AND a
        // PlayerPrefs.GetInt on every call, and two of the read sites below live inside per-frame
        // WaitUntil predicates. Folding once at StartWave also means the cap cannot change
        // underneath a wave that is already draining reinforcements against it, which would let
        // live > cap read as a defect in the very trace that reports it.
        //
        // -1 means "not folded yet" (before the first wave, and in an editor-instantiated
        // manager), and answers the authored value - never 0, which every site below treats as
        // UNCAPPED and would release the whole roster at once.
        private int _maxSimultaneousFolded = -1;

        /// <summary>
        /// The concurrency cap actually in force: the serialized value with WO-1773's remote
        /// percent folded in. 0 still means UNCAPPED, exactly as the serialized field does.
        /// </summary>
        private int MaxSimultaneous
            => _maxSimultaneousFolded >= 0 ? _maxSimultaneousFolded : _maxSimultaneousEnemies;

        // ── WO-1773: per-wave hero-damage accounting for the wave-end summary line ────────
        // Snapshotted at StartWave, diffed at CompleteWave. HeroHealth owns the running total;
        // this only remembers where the wave began, so nothing here can drift from that counter.
        private float _heroDamageAtWaveStart;
        private bool  _heroDamageSnapshotValid;

        // WO-1773: the roster total the live smart path actually generated for the current wave,
        // post endless count-scale. -1 = not generated by the smart path (a flat/batch wave, or
        // nothing spawned yet), which the summary line reports as "n/a" rather than as 0.
        private int _lastWaveRosterTotal = -1;
        ...
        private float EndlessCountScale(int waveId)
        {
            ...
            // WO-1773: this CAP is the numerically dominant half of "wave 176 is wave 60" — it was
            // measured binding from wave 60, 116 waves before the tester's run. FoldCountCap applies
            // a remote-tunable PERCENT and returns the authored cap UNCHANGED at the shipping
            // default (100), so with no database row the scale below is exactly what it was. A
            // non-positive authored cap still means "uncapped" and is handed back untouched.
            cap = WaveDifficultyTunables.FoldCountCap(cap);
        ...
        private void StartWave(int waveId)
        {
            ...
            // predicates never touch the rail (RemoteTunables.Int costs a PlayerPrefs read).
            _maxSimultaneousFolded = WaveDifficultyTunables.FoldMaxSimultaneous(_maxSimultaneousEnemies);

            // WO-1773: remember the hero's running damage-taken total so the wave-end summary can
            // report what this wave actually cost him. The external tester's report was "nothing can
            // damage him he can just stand there" and no capture has ever measured that number, so
            // the wave loop now prints it every wave whether anyone asked or not.
            var heroForSnapshot = HeroHealth.Instance;
            _heroDamageSnapshotValid = heroForSnapshot != null;
            _heroDamageAtWaveStart = heroForSnapshot != null ? heroForSnapshot.TotalDamageTaken : 0f;

            // Reset to "not a smart-composed wave" HERE rather than trusting the spawner to. Left
            // stale, a flat/batch wave would inherit the PREVIOUS wave's roster total and the
            // summary line would report a number from a wave that had already ended - a plausible
            // lie in a log, which is worse than "n/a".
            _lastWaveRosterTotal = -1;
        ...
        private bool SpawnSmartComposedWave(int waveId, WaveDef wave)
        {
            ...
            // WO-1773: remember the ROSTER TOTAL this wave actually generated, for the wave-end
            // summary line. Captured AFTER the endless count scale is applied, so it is the number
            // of bodies the wave will really field rather than the pre-scale generated count.
            _lastWaveRosterTotal = composition.TotalCount;
            ...
            if (MaxSimultaneous > 0 && budget <= 0)
            {
                ...
                FlowTrace.Warn("Wave",
                    ...
                    $"({_liveEnemies.Count}/{MaxSimultaneous}) — releasing NOTHING now and " +
            ...
            FlowTrace.Step("Wave",
                ...
                $"SHARED budget of {budget} (cap {MaxSimultaneous}, live {_liveEnemies.Count}, " +
            ...
            if (MaxSimultaneous > 0 && liveNow > MaxSimultaneous)
                FlowTrace.Fail("Wave",
                    $"wave {waveId}: CONCURRENCY CAP BREACHED — {liveNow} live enemies against a cap of " +
                    $"{MaxSimultaneous} after ONE SpawnWave call across {sidesThisWave} side(s). " +
                ...
                FlowTrace.Step("Wave",
                    $"wave {waveId}: post-spawn live={liveNow}/{(MaxSimultaneous > 0 ? MaxSimultaneous.ToString() : "uncapped")} " +
            ...
            if (deferred.Count > 0)
            {
                ...
                _reinforcementDrainUnscaled = Time.unscaledTime;   // WO-1308: a fresh hold is not a stale one
                FlowTrace.Step("Wave",
                    $"wave {waveId}: concurrency cap {MaxSimultaneous} released {squad.Count} now, " +
        ...
        private int SmartSpawnBudget()
            => SmartEnemySpawner.BudgetFor(MaxSimultaneous, _liveEnemies.Count);
            ...
            List<WaveCompositionEntry> deferred, int waveId, EnemyWaveComposition source)
        {
            ...
            while (deferred.Count > 0)
            {
                ...
                int budget = MaxSimultaneous <= 0 ? int.MaxValue : SmartSpawnBudget();
                if (budget <= 0)
                {
                                         ...
                                         EndlessPressureRole pressureRole = EndlessPressureRole.None)
        {
            ...
            for (int i = 0; i < Mathf.Max(0, batch.Count); i++)
            {
                ...
                if (MaxSimultaneous > 0 && _liveEnemies.Count >= MaxSimultaneous)
                {
                    await UniTask.WaitUntil(
                        () => _liveEnemies.Count < MaxSimultaneous
                              || _phase != WavePhase.Active
                              || TownSuspension.SuspendedFor(this));
                    if (_phase != WavePhase.Active) return;
        ...
        /// <summary>
        /// WO-1773 — THE ONE PER-WAVE DIFFICULTY SUMMARY. Prints, once per cleared wave, the five
        /// numbers that would have settled this ticket's whole RCA from a single log read instead of
        /// a frame-by-frame video analysis: the wave number, the enemy HP and contact-damage
        /// multipliers actually in force, the roster size, and how much damage the hero actually
        /// TOOK across the wave.
        ///
        /// <para>WHY THIS LINE EXISTS AT ALL (CLAUDE.md section 12). The report was "at the level he
        /// is nothing can damage him he can just stand there", and answering it cost a pixel scan of
        /// 146 one-second frames plus 80 tenth-second frames, because no log line anywhere said what
        /// a wave's multipliers were or whether the hero had been hit. It now does, on every wave,
        /// unprompted.</para>
        ///
        /// <para>ONE LINE PER CLEARED WAVE - not per frame and not per enemy - so <c>Step</c> is the
        /// correct shape here rather than the 4-arg <c>Measure</c> or a <c>Throttle</c>, and it
        /// cannot contribute to the logcat-ring eviction that destroys boot evidence.</para>
        ///
        /// <para>Guard-wrapped: a summary line is diagnostics, and diagnostics must never be able to
        /// break the wave-clear reward path it is reporting on.</para>
        /// </summary>
        private void LogWaveDifficultySummary(int cleared)
        {
            Guard.Try("Waves", "per-wave difficulty summary", () =>
            {
                var curve = EnsureScalingCurve();
                float hpMult  = curve.HpMultiplier(cleared);
                float dmgMult = curve.DamageMultiplier(cleared);
                int bandStart = WaveScalingCurve.GrowthBandStart(curve.HpCurve);

                var hero = HeroHealth.Instance;
                string damageTaken;
                if (hero == null)
                    damageTaken = "no hero resolved";
                else if (!_heroDamageSnapshotValid)
                    damageTaken = $"{hero.TotalDamageTaken:F0} lifetime (no wave-start snapshot)";
                else
                    damageTaken = $"{(hero.TotalDamageTaken - _heroDamageAtWaveStart):F0}";

                string roster = _lastWaveRosterTotal >= 0
                    ? _lastWaveRosterTotal.ToString()
                    : "n/a (not a smart-composed wave)";

                string band = bandStart == int.MaxValue
                    ? "curve is not post-clamped, so no growth term applies"
                    : $"growth band starts at wave {bandStart}";

                FlowTrace.Step("Waves",
                    $"WAVE SUMMARY wave={cleared} enemyHpMult=x{hpMult:F2} enemyDmgMult=x{dmgMult:F2} " +
                    $"roster={roster} concurrencyCap={(MaxSimultaneous > 0 ? MaxSimultaneous.ToString() : "uncapped")} " +
                    $"heroDamageTaken={damageTaken} ({band}). " +
                    "heroDamageTaken=0 with a non-trivial roster is WO-1773's symptom, not a clean clear.");
            });
        }
        ...
        private void CompleteWave()
        {
            ...
            LogWaveDifficultySummary(cleared);
