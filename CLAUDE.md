...
**⚠ THE GATE COUNTS DIFFERENTLY FROM THE ONE-LINER ABOVE (2026-09-09, WO-1096).** The one-liner
counts every brace in the file. `CompileGate.BraceBalanced` (`Assets/Editor/CompileGate.cs:896-948`)
skips comments and string/char literals with a plain state machine and has NO interpolated-string
model: a `"` inside a `$"...{ cond ? "a" : "b" }..."` hole ends the string, the rest scans as code,
and a file can read 210/210 raw and 175/174 at the gate, which withholds `COMPILE_GATE_OK`. Before
any gate run, ALSO run the port of the gate's exact rule:
`python tools/gate_brace.py $(git status --short | grep '\.cs$' | awk '{print $2}')` (exit 0 =
clean). When a `FlowTrace` string needs nested quotes inside an interpolation hole, compute the parts
into locals first.
