...
- **FIELDS (WO-1430 RESULT § §2.1–2.2):** `CosmeticCatalog.IsAchievementUnlock` (file:line `Assets/_Modules/Cosmetics/CosmeticCatalog.cs:48-51`) is the one definition; `CosmeticOwnershipService.GrantAchievement` now refuses non-achievement rows. `DailyQuestService.HeroRequirementMet` (`Assets/_Modules/Core/Quests/DailyQuests.cs:80-86`) gates requiresHero. The other three fields are **RETIRED, not wired** — owner ruling 2026-09-10 on WO-1430 fields 3-5 (*"Drop them - remove the dead fields from the catalog; re-add each when a system needs it"*). `levelCurve` is gone from `EchoBalanceCatalog.EchoBalanceData` and from BOTH `echoes-balance.json` twins; `visibilityRule` and `expiry_behavior` are gone from `Core/Data/CardCollectionCatalog.cs` (they were DTO members with zero authored rows). ⚠ The **server** side of `expiry_behavior` is deliberately untouched and still live (`api/schema.sql`, `api/_lib/catalog-read.js`, `api/admin/showcase-finalize.js`), so responses still carry the key — it is now an ignored unknown member. Their absence is pinned by `AuthoredFieldReaderRegression` case `[retired-field-stays-retired]`, which REDS if either a declaration or a canonical-json key comes back.
...
| Area | Types | Notes |
|---|---|---|
| **HudModel/** (`DeNelle.Core.HudModel`) | `HudModels.cs` (679L — the 11 Core HUD models + `IHudModel` facade + holder; producer-only mutators fire `Changed` + `[Flow:HUD]`), `HudModelTypes.cs` (shared enums/record structs; contract FROZEN per WO541_MODEL_API.md), `PostureSignals.cs` (calm→hostile(prebattle\|activebattle\|postbattle) battle-arc signals for the HUD kit), `WaveCounterMath.cs` (WO-1864 — `Remaining(live, held)` + `WaveRosterTracker`; the player-facing wave counter is `LiveEnemies.Count + WaveManager.HeldReinforcements`, **never** the field list alone, because the WO-1113 concurrency cap pins the field AT the cap). | WO-541 model layer behind `CoreServices.HudModel`. ⚠ `WaveModel.EnemiesLive` was **renamed `EnemiesRemaining`** 2026-09-18 (WO-1864) — the one amendment to the otherwise-frozen WO541 contract, and `EnemiesTotal` now means the wave ROSTER, not the live list's length. |
...
- **Localization (WO-1605):** every migrated surface calls the package-free
  ...
  The checkpoint manifest has 6,034 rows: 416 `keyedEntry`, 85 `keyedCall`, 5,517 report-only/unarmed
