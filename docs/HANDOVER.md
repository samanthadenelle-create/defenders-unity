> **2026-09-09 WO-1605 (ACTIVE GOAL):** Through commit `50cba5715`, one global Core facade and Unity Localization adapter enforce an exact 10-region change contract and expose a six-locale player beta (`en`, `es`, `pt-BR`, `de`, `fr`, `ru`). Arabic, Japanese, Korean, and Simplified Chinese stay parity-required but hidden pending RTL/CJK readiness. All ten canonical source/mirror catalogs carry 414 keys; the six enabled Unity tables carry 2,484 entries total. Calm navigation, the complete Heart HUD, and the two-state combat Flee control use domain wrappers, with typed named arguments where values must reorder naturally. Store has 39 translated/staged keys across purchase-gate, Pi, wallet-mirror, commerce-state, and the safe seven-key presentation cohort (`storeBandGap`, `storeBandGapSub`, `storeBandBasket`, `storeSpotlightEmpty`, `storeLedgerHeading`, `storeCardOwned`, `storeCardGap`). This remains data staging only: runtime still uses legacy `StoreStrings`/canon and Store is not fully localized. Presentation claims involving offers/patronage, value or balance comparisons, and ambiguous locked states are held for product/formatting review; the four trust-strip claims are held because they are not uniformly accurate across payment channels, and the covenant remains English raster art. The manifest is 6,031 rows: 414 `keyedEntry`, 83 `keyedCall`, 5,518 `literalCandidate`, and 16 `imageTextCandidate`; literal debt remains report-only/unarmed pending human classification. Every future player-facing copy change must update all required locale catalogs and mirrors, rebuild all enabled tables, and pass the localization tests. Translations may be marked limited/provisional pending review, but cannot be omitted from parity. The post-cohort focused gate is 7/7 and the fresh Seeker-size smoke is 18/18. The most recent clean full gate is 462/462; a later mixed-tree run is 460/463 on three unrelated HUD/source-registration assertions. The overnight procedure is `tools/localization/run-localization-overnight.ps1`; architecture canon is `docs/localization/architecture.md`. Store runtime cutover and broader visible-text migration remain open.
