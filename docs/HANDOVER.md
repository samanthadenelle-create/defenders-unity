> **2026-09-08 WO-1605 (ACTIVE GOAL):** Commit `14d355cee` establishes one global Core facade and Unity Localization adapter, an exact 10-region change contract, and a six-locale player beta (`en`, `es`, `pt-BR`, `de`, `fr`, `ru`). Arabic, Japanese, Korean, and Simplified Chinese stay parity-required but hidden pending RTL/CJK readiness. Every enabled table carries 358 keys; non-English choices say Beta. The focused gate is 5/5, the full gate 462/462, and the Seeker-size language smoke is 18/18. The overnight procedure is `tools/localization/run-localization-overnight.ps1`; architecture canon is `docs/localization/architecture.md`. Store/Village/Canon and broader visible-text migration remain open. The 5,966-row clean-tree inventory and exact 2,994-fingerprint literal baseline remain deliberately **unreviewed**; do not arm the ratchet or call the game fully localized.
