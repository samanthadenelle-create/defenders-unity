> **2026-09-09 WO-1605 (ACTIVE GOAL):** Through commit `df4627bae`, one global Core facade and Unity Localization adapter enforce an exact 10-region change contract and expose a six-locale player beta (`en`, `es`, `pt-BR`, `de`, `fr`, `ru`). Arabic, Japanese, Korean, and Simplified Chinese stay parity-required but hidden pending RTL/CJK readiness. Each enabled table carries 396 keys; calm navigation, the complete Heart HUD, and the two-state combat Flee control use domain wrappers, with typed named arguments where values must reorder naturally. Store's 21 purchase-gate, Pi, and wallet-mirror keys are translated in all ten catalogs and generated into the six enabled tables. Pi no longer carries the obsolete guest-price threshold, wallet-unavailable copy no longer implies a whole-build outage, and the Spanish/Portuguese/German HUD Store wordmarks no longer fall back to truncated English. This remains data staging only: `StoreStrings` still uses its legacy reader pending an atomic full-Store cutover. Generated Unity tables must exactly match canonical sources, and the tracked static fallback font covers every enabled-locale scalar. The post-cohort focused gate is 7/7 and the fresh Seeker-size smoke is 18/18; visual inspection confirms `DER NACHTMARKT` is centered without ellipsis. The most recent clean full gate is 462/462; a later mixed-tree run is 460/463 on three unrelated HUD/source-registration assertions. The overnight procedure is `tools/localization/run-localization-overnight.ps1`; architecture canon is `docs/localization/architecture.md`. Store runtime cutover and broader visible-text migration remain open. The 6,013-row clean-tree inventory and exact 2,994-fingerprint literal baseline remain deliberately **unreviewed**; do not arm the ratchet or call the game fully localized.
