> **2026-09-08 WO-1605 (ACTIVE GOAL):** Through commit `f501a38df`, one global Core facade and Unity Localization adapter enforce an exact 10-region change contract and expose a six-locale player beta (`en`, `es`, `pt-BR`, `de`, `fr`, `ru`). Arabic, Japanese, Korean, and Simplified Chinese stay parity-required but hidden pending RTL/CJK readiness. Each enabled table carries 375 keys; calm navigation, the complete Heart HUD, and the two-state combat Flee control use domain wrappers, with typed named arguments where values must reorder naturally. Generated Unity tables are now required to exactly match canonical sources, and the tracked static fallback font covers every enabled-locale scalar. The focused gate is 7/7, the full gate 462/462, and the Seeker-size smoke is 18/18 with live Heart/Flee assertions. The overnight procedure is `tools/localization/run-localization-overnight.ps1`; architecture canon is `docs/localization/architecture.md`. Store and broader visible-text migration remain open. The 5,992-row clean-tree inventory and exact 2,994-fingerprint literal baseline remain deliberately **unreviewed**; do not arm the ratchet or call the game fully localized.
