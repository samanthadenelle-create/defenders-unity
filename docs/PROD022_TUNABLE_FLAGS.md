...
| 32 | `raid.starterArmySize` | int | `10` | **TOTAL** free troops a save receives the first time it has a Barracks, split **5 Footmen + 5 Archers**. WO-1803 (2026-09-16), owner verbatim: *"Instead of giving them three troops, because that's shit, let's give them ten troops. Let's give them five footmen and five archers."* This knob is the total ONLY - `StarterArmyGrant.SplitComposition` derives the halves (even, remainder to Footmen), so no value here can produce a squad of one unit type. Granted **once per save** via the monotonic acquired-ledger key `grant.starter-army`, so a demolished-and-rebuilt Barracks is **not** a troop faucet, and an existing save that already took the 3-troop grant gets **nothing** (the ledger key is latched). `0` disables the grant. Clamped to `0..ArmyStorage.DefaultMaxArmySize` (**10**, the fresh-save army housing cap) at `StarterArmyGrant.ResolveCount` - read off that const, never a literal, so the knob ceiling and the housing cap cannot drift. | **Not a PROD-022 hypothesis - the FTUE lever the map opens with (WO-1374, §2):** *"A player starts with 200 gold but needs 1,650 to participate in the thing you're trying to teach them. That's basically putting a nightclub behind a velvet rope and handing the player twelve cents."* Three was that lever's first setting and the owner's verdict was *"that's shit"*: ten is a squad that can take the 9-defender first camp (`raider_camp_small`, garrison 9 - so the grid stops printing "Outmatched - Army 9 advised" on it), and the 5/5 split is the first thing in the game that teaches composition. Whether **ten** is right is a felt question about the **first ten minutes of the game**: the most expensive ten minutes to get wrong and the most expensive to iterate on through a rebuild. ⚠ A fresh save is **10/10 housing** after the grant, so training is closed until the barracks-tier `armyCapBonus` perk raises the cap - the CoC shape (the camp is full, upgrade it), recorded deliberately rather than papered over. ⚠ Correct under BOTH sides of the blocked troop-cost fork, which is why it shipped while the gold half did not. |
...
| 76 | `raid.lossPctFail` | int | `100` (⚠ the RULING, not today) | **PERCENT of the SURVIVORS a FAILED raid loses permanently** (WO-1810) - clock expiry, a wiped warband, the stranding watchdog, or the hero-death settlement. Troops **KILLED** on the field are dead on every outcome and are deliberately **not** on the rail ("any troop killed is dead" is a ruling, not a dial); this is what happens to the ones still standing. `0` restores the pre-WO-1810 behaviour, in which a lost raid removed **nothing**. Clamped `0..100` at `RaidCasualtyPolicy`, read through `SpecFor` **before** `Int` so an unregistered key answers `100` rather than `0`. | **Not a PROD-022 hypothesis - the owner's ruling 2026-09-16, verbatim:** *"there is no cost to losing a raid"* / *"troops return a few injured which has no cost or time"* / *"maybe lose 100 on fail"*. ⛔ **THIS DEFAULT IS THAT RULING, NOT TODAY'S BEHAVIOUR** - same deliberate departure as #9/#19/#43. Measured on her Seeker (build 372984, 2026-09-16 19:59:01): a timed-out raid reported `deployed 10, survivors 7, wounded 3 ... recovery 1200s`, `TroopRecoveryService: recovery advanced; 3 still wounded`, and the army screen read **"Army is full. 10/10 slots used"** one minute later. The loss cost nothing and the killed healed for free. Whether a **total** wipe is too harsh on a first camp is the felt question a row answers instead of a 30-minute rebuild. |
| 77 | `raid.lossPctRetreat` | int | `60` (⚠ the RULING, not today) | **PERCENT of the SURVIVORS the player's OWN Retreat loses permanently** (WO-1810) - charged on *"whats left"* after the killed are already gone. The rest come home **healthy, with no recovery timer**. Rounded to the **NEAREST** troop with a **floor of one**, so a lone survivor of a retreat is never a free save (`ceil` was rejected: it takes 100% of two survivors and 71% of seven). Clamped `0..100`; `0` makes retreating free again. | **Not a PROD-022 hypothesis - the same 2026-09-16 ruling:** *"lose 60% on retreat"* / *"but any troop killed is dead so 60% of whats left"*. ⛔ Also a departure from today's behaviour, which was `0`. Its own row rather than a share of #76 because **the gap between the two is the entire reason to press Retreat** rather than run the clock out - if retreating is not visibly cheaper than failing, the button is decoration. |
| 78 | `army.dismissReturnPercent` | int | `50` (PROPOSED, the owner ruled the verb not the number) | **PERCENT of a troop's CATALOG gold value (`TroopDef.CostGold`) returned when the player DISMISSES a trained troop from the army screen** (WO-1811, owner 2026-09-16: *"either return them to gold or to a staged ready troop"*). Training itself charges **nothing** since WO-1387, so this is a price on the way OUT, not a refund of anything paid. `0` = dismissal is pure deletion; `100` = rebalancing is free. The other exit - **Reserve** - costs and pays nothing. | **Not a PROD-022 hypothesis.** Whether half is generous enough that a player will fix a wrong army rather than live with it is felt, not derived. |
| 79 | `town.captureSalvagePct` | int | `50` (the owner named 0.5; the rail carries no float) | **PERCENT of a ruined structure's CATALOG BUILD COST** paid as salvage when the player clears that ruin in her captured town. WO-1872 converts the taken Iron Bastion as the **destroyed camp** - its walls and watchtowers arrive as rubble where they fell instead of standing - and clearing each ruin is what funds her own layout. Floored per resource, so salvage can never round **up** past what the structure cost. Clamped `0..100` at `OwnedTownConstructionService.SalvagePct`, deliberately **not** `0..1000`: a ruin paying more than it cost to build would turn the captured perimeter into a resource printer. `0` makes clearing free but unpaid; `100` is a full refund. | **Not a PROD-022 hypothesis - the one number the owner named in WO-1872.** Owner, 2026-09-18, verbatim: *"I think we should load a destroyed camp and then clear the rubble and give them resources to make player designed layouts"*. Whether a field of ~168 ruins reads as a reward to harvest or a chore to grind is a felt question that is not knowable from source, and every answer would otherwise cost a build. |
...
| 57 | `raid.difficultyMultPctCamp1` | int | `100` (= the authored value, unchanged) | **PERCENT** on Camp I's **authored** `difficultyMultiplier` (WO-1763) - the one number that scales a raid defender's **HP and contact damage**, folded in exactly once at `RaidGarrisonSpawner.FoldDifficulty`. `100` short-circuits at the consumer and returns the authored float untouched, so an empty table is today **bit for bit**. It changes how hard each defender is, never **how many** there are. Read the authored baseline off `Assets/Resources/Data/Canonical/scene-configs.json` - this row deliberately restates no number from it. Clamped `25..400` at `RaidDifficultyTunables`; applies on the **next raid entry**. | **Not a PROD-022 hypothesis - the lever that did not exist.** Until WO-1763 the only way to make a camp harder was to edit a canonical JSON and ship a build, so "is this camp too easy" cost a ~30-minute rebuild per opinion. The felt evidence: the owner **nearly 3-starred the Iron Bastion with a level-4 hero**. |
| 58 | `raid.difficultyMultPctCamp2` | int | `100` (= the authored value, unchanged) | The same **PERCENT** for Camp II. `100` = today. Clamped `25..400`. | **Not a PROD-022 hypothesis** - its own row rather than one global dial, because the defect this ticket answers is that the camps do not **differ** enough. A single global multiplier moves all four together and can never separate them. |
| 59 | `raid.difficultyMultPctCamp3` | int | `100` (= the authored value, unchanged) | The same **PERCENT** for Camp III. `100` = today. Clamped `25..400`. | **Not a PROD-022 hypothesis** - see #58. The step between camps is what decides whether unlocking a harder raid reads as progress or as the same raid with more HP. |
| 60 | `raid.difficultyMultPctBastion` | int | `100` (= the authored value, unchanged) | The same **PERCENT** for the **Iron Bastion**. ⭐ **The knob this ticket was opened for:** as shipped, the Bastion's garrison block in `scene-configs.json` is a **field-for-field clone of Camp III's** on every difficulty axis, so the map's fourth and evergreen target fights exactly like its third. This is how the two stop being the same fight - **without a rebuild and without touching the authored JSON**. Clamped `25..400`. | **Not a PROD-022 hypothesis - the direct answer to a felt report.** How much harder the evergreen target should be than the camp it was cloned from is a question only playing it answers, and the answer will move more than once. ⚠ Per the **WO-1705 ruling**, a full 3-star Bastion clear triggers owned-town **capture** and a hero-down settles the cap at 2 stars, so raising this changes how often that tradeoff fires - in both directions. |
| 61 | `raid.levelOffsetCamp1` | int | `-999` (= the sentinel: use the authored value) | **REPLACES** Camp I's **authored** `levelOffset` - how far **above the player's own level** its defenders are rolled (WO-1763). `-999` is the **sentinel** meaning *use what `scene-configs.json` authors*, i.e. today exactly; **deleting the row does the same thing** and is the table's documented resting state. It **REPLACES, it does not ADD** - the number written **is** the offset. The spawner's `max(baseEnemyLevel, playerLevel + offset)` floor is untouched, so `baseEnemyLevel` still floors a low-level hero's raid. Any non-sentinel value is clamped `-5..20` at the consumer. | **Not a PROD-022 hypothesis - the second of the two difficulty axes,** and the one that bites hardest late: `GarrisonStatBlocks.ApplyLevelScale` adds ~8% HP and up to ~4% contact damage per level **on top of** the multiplier. Kept separate from #57 because "more levels" and "tougher for its level" feel completely different to fight. |
| 62 | `raid.levelOffsetCamp2` | int | `-999` (= the sentinel: use the authored value) | The same **REPLACEMENT** offset for Camp II. `-999` = today. Clamped `-5..20` otherwise. | **Not a PROD-022 hypothesis** - see #61. Per camp rather than global, for the same reason the multipliers are. |
| 63 | `raid.levelOffsetCamp3` | int | `-999` (= the sentinel: use the authored value) | The same **REPLACEMENT** offset for Camp III. `-999` = today. Clamped `-5..20` otherwise. | **Not a PROD-022 hypothesis** - see #61. |
| 64 | `raid.levelOffsetBastion` | int | `-999` (= the sentinel: use the authored value) | The same **REPLACEMENT** offset for the **Iron Bastion**. ⭐ The second half of the Bastion pair. **REPLACE, NOT ADD** - the number written is the offset, not a delta on the authored one. `-999` = today. Clamped `-5..20` otherwise. | **Not a PROD-022 hypothesis - the axis the felt report actually named.** A level-4 hero met the Bastion's authored offset and nearly cleared it; raising the offset is how a high-level hero keeps meeting a fight rather than a formality. ⚠ It **compounds** with #60 - levels raise HP and damage on top of the toughness percent. |
| 65 | `town.regenSuppressSecondsAfterHit` | int | `0` (= OFF, no combat gate - today) | **SECONDS** after the hero last actually **LOST HP** during which the town safe-zone regen stays **off** - the out-of-combat rule. At `0` the regen has no combat check of any kind and runs mid-wave with an enemy in melee contact. ⭐ **WO-1773 names this the PREREQUISITE for every other dial in this family:** the regen is a **fraction of MaxHp per second**, so it scales with gear while incoming contact damage is capped by the wave curve's clamp - the gap **widens with every upgrade and can never be outgrown**. The between-waves top-up and the FTUE 1-HP recovery both survive this shape with no carve-out. Clamped `0..120`; the first-time tutorial is never gated. | **Not a PROD-022 hypothesis - the direct answer to an external tester's felt report,** relayed verbatim by the owner 2026-09-16: *"at the level he is nothing can damage him he can just stand there"*. WO-1773 measured the hero plate across 146 one-second frames and 80 consecutive tenth-second frames of a wave-176 run: the bar never moved, with a Cave Troll in melee contact. How many seconds out of combat should buy a top-up is a feel question that will be re-tuned several evenings in a row. |
| 66 | `town.regenPctDuringWave` | int | `100` (= the full rate - today) | **PERCENT** of the normal town regen rate that runs while a town wave is **ACTIVE**. `100` = today, bit for bit (the consumer short-circuits and never round-trips the fraction). `0` stops town regen for the whole wave. It **composes** with #65 and **#65 WINS** - a hero being hit right now is in combat whatever the wave phase says. Clamped `0..100`; **above 100 is refused on purpose**, because regenerating faster inside a wave than between waves is the opposite of the defect. | **Not a PROD-022 hypothesis - the second shape WO-1773 offered for the same dead step.** Kept alongside the timer rather than instead of it: *"no healing during a siege"* and *"no healing for six seconds after a hit"* feel completely different to play, and which one is right is not knowable from source. |
| 67 | `wave.hpGrowthPctPerWave` | int | `0` (= no growth - today) | **HUNDREDTHS of enemy HP multiplier ADDED per wave** beyond the point where `WaveScalingCurve` stops growing on its own. `5` = +0.05 on the multiplier every wave past the band. `0` = today: the curve's `postWrapMode` is `Clamp`, so **wave 176 is numerically wave 60**. ⚠ **ADDITIVE on the clamped multiplier, NOT multiplicative** - the two shapes diverge hard at high waves, and the ticket's arithmetic table (which is what a seed gets picked off) is the additive column. The band start is **read off the curve's last keyframe**, never written down. Clamped `0..100` per wave, with an absolute ceiling on the effective multiplier because the wave number itself has no clamp. | **Not a PROD-022 hypothesis - the owner's own direction,** 2026-09-16: *"enemies need to really start scaling with wave i thought"*. The shape is deliberately **the dragon's**: its HP and damage already grow linearly and uncapped per return while the entire regular roster is clamped, so this applies a pattern the codebase already has to the rest of the wave. |
| 68 | `wave.dmgGrowthPctPerWave` | int | `0` (= no growth - today) | The same **ADDITIVE** growth for **enemy contact damage**. `0` = today. ⭐ **This is the axis that decides whether a high-level hero can stand still.** The ticket's arithmetic puts one heavy melee enemy at parity with the town regen near `5` and clearly ahead by `10` - **but only once #65/#66 are seeded**, because until then every row up to 5 still reads as a net heal. Clamped `0..100` per wave. | **Not a PROD-022 hypothesis - the half of the scaling direction that is actually about threat.** Scaling HP alone lengthens fights without adding danger, which is the least interesting way to make a game harder, so the two knobs are independent and will not be tuned to the same number. |
| 69 | `wave.maxCountPct` | int | `100` (= the authored ceiling, unchanged) | **PERCENT** on the authored **roster ceiling** - how many bodies one wave may field in total. At `100` that ceiling binds **from wave 21**, so a wave-176 roster is the same size as a wave-21 one. This is the **cheap half** of the owner's *"massive swarms at all sides"*: the roster drains through the concurrency budget as reinforcements, so a bigger roster costs arrival **time**, not frame time. Clamped `25..1000`, floored at one body so a low percent can never produce an empty wave (which the clear logic reads as already cleared). | **Not a PROD-022 hypothesis - the owner asked for swarms** and the spawn side already uses **all four gates from wave 10**, so the missing ingredient is volume, not coverage. Whether a longer, denser siege reads as epic or as tedious is a felt question. |
| 70 | `wave.countCapPct` | int | `100` (= the authored cap, unchanged) | **PERCENT** on the authored **endless count cap** - the multiplier endless mode applies once the authored schedule runs out. ⭐ **This is the cap that binds FIRST:** WO-1773 measured it binding **from wave 60**, a hundred and sixteen waves before the tester's run, which is most of why 176 and 60 are the same fight. Raising it is the single largest available change to roster size. Clamped `25..1000`; an authored cap of `0` means *uncapped* and is handed back untouched. | **Not a PROD-022 hypothesis - the numerically dominant half of dead step B.** It is a **percent** rather than a replacement so no number from `waves.json` is restated on the rail, which is the stale-copy failure CLAUDE.md keeps recording. |
| 71 | `wave.maxSimultaneousPct` | int | `100` (= the scene's value, unchanged) | **PERCENT** on the **on-screen concurrency cap** - how many wave enemies may be alive at once, split across the four gates. ⛔ **THIS IS A PHONE FRAME BUDGET, NOT A DIFFICULTY DIAL,** and it is the only knob in this family that can regress the Seeker build's frame rate: the spawner partitions a single budget across sides precisely so N gates can never exceed the one-gate cap, after the WO-1113 phone frame-rate cliff. **Raising it requires measured `FlowTrace.Measure("Perf", ..., 4f, 1f)` evidence naming the dominant cost in ms, before and after, on the device** (CLAUDE.md §12). Clamped `25..400` - tighter than #69/#70 for that reason. An authored `0` means *uncapped* and is handed back as `0`; a real cap never rounds down to `0`, which would read as uncapped. | **Not a PROD-022 hypothesis - the EXPENSIVE half of "massive swarms at all sides".** On the rail so the real device ceiling is found by playing rather than by a rebuild per guess, and so a bad value is one row-edit away from being undone on a build already in a tester's hands. |

| 72 | `dungeon.lanternDrainPerSecX100` | int | `50` (= today's 0.50 oil/s) | Oil burned per second by the dungeon lantern, **times 100** (the rail carries no floats). `50` = `0.50`/s = **200 s a flask**, which is exactly what `Assets/Resources/Data/Canonical/dungeon-balance.json` authors, so an empty table burns identically. `33` ≈ 300 s, `25` = 400 s. ⛔ **`maxOil` is deliberately NOT on the rail** - it is the meter's 100 % and the denominator of the low-oil `0.25`, darkness-latch `0.12` and min-light `0.35` fractions, so moving it silently moves three systems. Tune the **rate**. A row **at** `50` leaves `dungeon-balance.json` as the single authority; only a deviation overrides it (`Lantern.ApplyBalanceData`). Floored at `0.01`/s. | **Not a PROD-022 hypothesis - the owner's report 2026-09-16, verbatim:** *"we never really ever go over the mechanics of the torch in so nobody understands why the torch runs out and why just become suddenly dark."* WO-1805 Lane A teaches the mechanic; this row is how "is 200 s of light fair for an eleven-room dungeon" gets answered on device instead of by a ten-minute rebuild per opinion. |
| 73 | `dungeon.lanternOilStoneRefillPct` | int | `100` (= a stone tops the flask) | **Percent** of a full flask one authored oil stone returns. `100` is arithmetically identical to what `Lantern.CheckOilStones` has always done (`_oil = _maxOil`) from any starting level. ⚠ Each stone remains **ONE USE per visit** and this row cannot change that. Clamped `1..100`. | **Not a PROD-022 hypothesis - the recourse half of WO-1805.** `dg_starter_loop` authors **one** stone for **eleven** rooms, so this row and #72 together are that dungeon's entire light budget. Whether finding a cache should read as a full reprieve or a partial one is felt, and it used to be a compiled assignment. |
| 74 | `dungeon.lanternStillRefillPct` | int | `40` (= `ComposedOilStill.RefillFraction` 0.40f) | **Percent** of a full flask one **emergency field distillation** returns - the one-use still that spends 1 `ing_oil_flask` + 1 `tattered-cloth`. `40` is ~80 s at today's drain. `100` makes a flask-plus-cloth a complete refill. ⚠ The still is still attached **1:1 to each oil stone** (WO-1805 Lane B, SPEC), so raising this pays more **where one exists** - it does not add more of them. Clamped `1..100`. | **Not a PROD-022 hypothesis - the owner's** *"we need a mechanic, they can actually get more materials and use it"* **(2026-09-16), as far as one number can carry it.** The rest of that sentence needs new seams (a bag-side oil consumable, a still placement rule) and stays a spec. |
| 75 | `dungeon.lanternFinalWarningSec` | int | `30` (= `Lantern.DefaultFinalWarningSeconds`) | **Seconds** before empty at which the flame begins its final visible collapse: reach falls to the `1.35` u safety halo, the wick flutters, and a **linear fog wall** closes from the scene's own distances to **0.45..3.2 m**. ⚠ **THIS IS THE "SUDDENLY DARK" KNOB.** Raise it and the same collapse spreads over a longer, readable ramp; `0` removes the ramp and the dark arrives at empty with no warning. A row **at** `30` leaves `dungeon-balance.json` and the serialized field alone. Floored at `0`. | **Not a PROD-022 hypothesis - it is the owner's word "suddenly" as a number.** The `_minOilLightFraction` `0.35` floor does **not** protect her: the collapse writes a `1.35` u halo behind a `3.2` m fog wall on top of it, and `ComposedAmbushDirector`'s darkness multiplier arms just after. This row decides how much notice the player gets. |
...
### The `store.*` family — the STOREWIDE SALE, and it is **server-only** (WO-1799)

Owner, 2026-09-16, verbatim: *"can we run a 30% deal?"* Before this the **only** discount in the game
was the per-wallet **shortfall** discount (2000 bps, once per 7 days, `api/purchases/quote.js`). There
was no way to run a sale at all, and the shipped APK cannot change without a build.

| key | kind | resting state | what it does |
|---|---|---|---|
| `store.saleBps` | int | **absent / 0 = NO SALE** | basis points off the USD anchor of **every quotable SKU on every server-priced rail**. `3000` = the owner's 30% deal. **Clamped 0..7000 on the read** (`api/_lib/store-sale.js`), so a fat thumb cannot give the store away. |
| `store.saleEndsAtEpochMin` | int | absent / 0 = runs until cleared | epoch **MINUTES** at which the sale stops on its own. |

> ### ⛔ THESE TWO ARE THE FIRST `serverOnly` ROWS ON THIS RAIL, AND THE SIX-PLACE RULE BELOW DOES **NOT** APPLY TO THEM.
> Of the six sources the next section says to change together, a server-only knob touches exactly
> **two**: the `TUNABLE_KEYS` allowlist (#2) and this document (#3). It has **no**
> `RemoteTunables.Registry` entry (#1), **no** `ExpectedDefaults` row (#4), **nothing** in the
> generated manifest (#5) and **no Command Center card** (#6) — every one of those omissions is
> deliberate and each is pinned by a test.
>
> **Why:** a sale percentage the Unity client could read would be a **second authority about money on
> a device we do not control** — the duplicated-state failure CLAUDE.md §2/§5/§16 each record a scar
> from, arriving through the money door. So `api/client-tunables.js` **withholds every `serverOnly`
> key from its public payload** (the exclusion set is *derived* from the allowlist, never typed), and
> the Command Center never offers the lever, because that page's own `OUT_OF_SCOPE_NOTICE` says prices
> are never editable there and means it. The marker is read off the **allowlist spec**
> (`{ key: 'store.saleBps', kind: 'int', serverOnly: true }`), which is what exempts it from the
> build-registry join in `api/_lib/tunable-manifest.js` — and from that alone.

**⚠ WHY THE END TIME IS EPOCH *MINUTES* AND NOT SECONDS.** `normalizeValue()` accepts at most **nine
digits**; epoch seconds is **ten**, so a seconds value is **REFUSED AT WRITE TIME** and looks like a
mistyped key. Minutes is eight digits and also fits a C# `int`. ⛔ **Do not widen the `\d{1,9}` rule to
make seconds fit** — it widens every key on the rail.

**⚠ AND THERE IS DELIBERATELY NO `store.saleLabel` ROW.** The rail stores **ints and bools only**, so
the label is DERIVED once, server-side — `3000` → `"30% off"` (`store-sale.saleLabel`) — and served
exactly where the shortfall label already goes. One pattern, no operator typo, and the client still
performs no percentage arithmetic.

**The combination rule: MAX, never additive.** A player who qualifies for the shortfall discount during
a sale gets the **better of the two**, and a tie goes to the sale (which leaves their once-per-7-days
shortfall window **unspent**). `2000 + 3000` would be a 50%-off pack nobody authored, on every SKU at
once.

**Which rails it reaches.** SKR (`api/purchases/quote.js`) and Pi (`api/pi/quote.js`) — both
server-priced, both through the one `readStoreSale` helper. ⛔ **Google Play is EXCLUDED and cannot be
included here:** a Play purchase is priced by its in-app product in **Play Console** and charged by
Google, so the server never issues an amount a sale could discount. Running the deal on Play is a Play
Console action. There is no USDC or SOL purchase rail at all.

Worked example, and the undo:

```powershell
node tools/client-tunables.mjs set store.saleBps 3000          # 30% off, storewide, now
node tools/client-tunables.mjs clear store.saleBps             # back to full price
```
