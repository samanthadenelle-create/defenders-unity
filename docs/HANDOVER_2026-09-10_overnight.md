...
2. WO-1701 (naked Blink mage on WebGL): DONE after this section was first written - lane merged, gated (wave12-compile1 COMPILE_GATE_OK, wave12-reg2 REGRESSION_OK 507/507, one oracle anchor re-pointed in AudioStartupBoundedRegression because AddressableRegistered went public), WebGL rebuilt, R2 parity green, deployed to defenders-pi.vercel.app, committed a4c0e7cd1, pushed. NOT device-proven: the next Seeker session in Pi Browser must show the Mage in Mage art. The lane named three follow-ups with no ticket yet: HeroTextureLoader.cs:79 still calls WaitForCompletion unguarded (the Enemies/OrcTex throw), StoryCompanionInjector requests slugs the prewarm does not warm, and a downloaded-but-unloadable hero body Fails loudly and continues (owner call). The regression method name is DeNelle.Editor.DataRegression.RunAll (a bare DataRegression.RunAll fails with 'class could not be found').
