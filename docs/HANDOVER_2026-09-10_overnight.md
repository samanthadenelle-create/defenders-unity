...
## 8. Evening Pi session (2026-09-10 16:00-18:50 local, owner live on Pi Desktop + the Seeker in Pi Browser)

**Outcome: the Pi testnet purchase PASSED at 23:42Z** (approve OK, complete OK with txid 51a1fd62...bb89, grant journalled) - the Pi Developer Portal's last checklist item before mainnet. Owner: "completed payment".

### Commits (all on dev, pushed)
- 5b7d55beb WO-1699 Pi Desktop (App Studio) recognised as a Pi host: `PiIsPiBrowser` gains the `PiNetwork/` UA token and the framed `.minepi.com` referrer (the SDK's own host channel); page template gains the host-fingerprint crumb and loader-progress crumbs; new validation-key.txt.
- 8e98d08fc research memo `docs/reference/PI_AD_NETWORK_EXPANSION_2026-09-10.md` (ad network = mainnet-only portal application, no SDK change; owner: "docs say no till mainnet"); WO-1701 minted; banner 1702.
- b5da08733 WO-1700: on the Pi channel a server-verified Pi sign-in is the durable identity (`PurchaseGate.HasVerifiedPiIdentity`); nothing had been buyable on Pi since WO-1386 (09-04 23:32). Gate: wave11-compile1 / wave11-reg2 506/506; WebGL wave11b; R2 parity 279.
- d572b186c WO-1700 FIXED with the captured purchase.
- (pending) WO-1701 lane: WebGL hero falls back to the Blink base because HeroAssetLoader.cs:94-95 uses WaitForCompletion, which WebGL refuses even on a cached bundle (captured 21:54:24Z). Lane running in a worktree at the time of writing; the lead registers its regression in DataRegression.cs, gates, rebuilds WebGL, deploys.

### Pi platform facts learned today (each proven, see WO-1699/1700 for the captures)
- Pi Desktop App Studio UA: `... PiNetwork/0.6.3 Chrome/134 Electron/35.2.0 ...`, referrer https://app-cdn.minepi.com/, framed. Phone: `... PiBrowser/1.17.1`, same referrer. pi-sdk.js resolves its host from document.referrer + window.parent.postMessage, never the UA.
- TWO Pi app records exist. Pi Browser binds https://defenders-pi.vercel.app to the app whose consent sheet reads "Defenders of the Realm"; the App Studio record "Echoes Of Elarion" (registered today, Testnet, validation key 155fe4c5...e881 served at /validation-key.txt) is separate. Payments are created under the URL-bound app; its key is the only one that can see them (direct api.minepi.com calls: Echoes key -> 404 payment_not_found and empty incomplete_server_payments; Defenders key -> 200). `PI_NETWORK_API_KEY` in Vercel (Production + Preview) now holds the Defenders of the Realm key; before today it was an 11-character placeholder, so no server-side approve had ever run with a real key. The owner ran both production redeploys (the seat's classifier refuses `vercel redeploy`); current production dpl f0b86w6sk. DO NOT delete the "Defenders of the Realm" app record.
- Mainnet = a new app record + new key (network fixed at registration), flipped together with `PiEnvironment.Sandbox` (WO-1325). Ad Network = portal application after the mainnet listing; code already matches the docs (memo above).
- The 15 s nativeFeatures timeout on the Seeker is our own probe ceiling (`WebGLPiPlatform.cs` ProbeTimeoutMs); ads simply read as unavailable.

### Open, no ticket yet (owner remarks, captures noted)
- 65% loader crash on the Seeker in Pi Browser: two boots at 21:17Z died in `unity-loading` with no pagehide (WebView kill shape); one later load reached 100% (jsHeapUsed 923 MB at 100% on the desktop panel; heap limit 1965 MB on the Seeker). Progress crumbs now land every 10% and every 1% from 60%. Feeds WO-1314 / WO-1484 (both parked pending exactly this measurement).
- "modal pop ups ... long tap to stay open" in Pi Browser: every panel in the trace opened and verified visible; the closing tap is not captured yet. Next capture: which panel, then read the POINTER-DUMP at that moment.
- Bug report #4 (21:55Z) arrived with note=0ch; the owner then named it: the naked Blink mage = WO-1701. Its screenshot also shows an untextured watchtower: `[Flow:VisualFactory] model not found ... 'Structures/Tower_Wooden_Watchtower'` and `'Structures/lumbermill'` at 21:48Z - a different loader, no ticket minted.
- Pi store copy: the Pi plate says "Connect a wallet" where "Sign in with Pi" is the remedy (`storePiWalletGate`); every shelf card reads "Not on sale in Pi yet" by design (WO-1069/WO-1323: only hearth-spark is Pi-quotable, in the spotlight).
- Pi app listing strings given to the owner: short description = the canon tagline "Echoes of a Forgotten Civilization"; privacy https://echoes-of-elarion.vercel.app/privacy; terms https://echoes-of-elarion.vercel.app/terms; app URL and dev URL both https://defenders-pi.vercel.app.
- The logcat capture from the Seeker sits in the session scratchpad (`pi_phone_logcat.txt`); nothing in it named a kill during the successful runs.

### Resume steps for the next CLI seat (the owner is out of weekly tokens as of 2026-09-10 18:50; nothing runs until her limit resets)
1. `git status -sb` - expect only `Assets/Resources/Localization/Fonts/ElarionLocaleFallback.asset` (pre-existing, not ours). HEAD is the docs commit that carries this section; origin/dev matches.
2. WO-1701 (naked Blink mage on WebGL): an Opus lane was dispatched at ~18:20 in the git worktree `.claude/worktrees/agent-a74b360b34c7b3fb0` (branch `worktree-agent-a74b360b34c7b3fb0`, based on f5d39acd1; `git -C <that path> status --short` shows its edits). If its worktree holds edits to `Assets/_Modules/Core/Addressables/HeroAssetLoader.cs`, `HeroContentPrewarmer.cs`, a new `Assets/Editor/Regression/HeroAssetLoaderWebGlRegression.cs`, the WO and a RESULT: copy them in (lead merge method in section 4), register the suite in `Assets/Editor/Regression/DataRegression.cs` after the last registration line, `python tools/gate_brace.py` on each .cs, compile gate + `DeNelle.Editor.DataRegression.RunAll` (the method is namespaced - a bare `DataRegression.RunAll` fails with "class could not be found", chain 51 did exactly that), then `build-webgl.ps1`, `tools2-ship.ps1`, `vercel deploy --yes --no-color` with VERCEL_PROJECT_ID=prj_qUmuwr8BN492oZH8yRuvPZMN3e0J VERCEL_ORG_ID=team_2PrmHqE5mM52aIrzPJNHmyEt, `vercel alias set <deployment> defenders-pi.vercel.app --scope team_2PrmHqE5mM52aIrzPJNHmyEt`, commit by explicit path, push dev. If the worktree is empty or gone, the ticket is READY and the brief is its section 3.
3. Then the owner's list, in her order: mainnet request in the Pi portal (her click; then a NEW app record + key -> set PI_NETWORK_API_KEY, flip `PiEnvironment.Sandbox` in the same commit, she runs the production redeploy); WO-1697 / WO-1698 READY; the six rulings in section 7; the open captures in section 8.
4. The Seeker logcat process started at 16:2x may still be running on this machine (`adb logcat -v time` redirected into the session scratchpad); kill it if `Get-Process adb` shows it.
5. Every visual ticket still gets a device frame; nothing in wave 10 or 11 is device-proven except the Pi purchase.
