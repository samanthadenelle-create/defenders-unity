...
**Status:** READY TO IMPLEMENT — BOUNCED 2026-09-18 20:40 by the owner's device frame `Logs/device/raid-fog-203029.png`: two grey puffs low over the base under a clear blue sky, NO visible ground fog (fog loops had been pool-SUSPENDED 10 s, and are too thin in daylight), arrows unproven. The lead flipped this FIXED on green markers without opening a frame - that was the defect in process. REDIRECT: (1) darken the raid sky itself (raid-scene camera tint + ambient drop, cloud layer spanning the sky, not two puffs); (2) fog as a low dense blue-grey layer across the whole yard, exempt from pool suspension; (3) occasional lightning flash from the storm prefab; (4) a frame with a tower arrow in flight. Does not flip past IMPLEMENTED until the lead has OPENED a raid frame showing all three. PRIOR: FIXED — ON SEEKER build `2026.09.18.375545` (installed 14:13, Firebase release 3dve2e518t3b8); owner felt-verify closes. PRIOR: FIXED PENDING DEVICE BUILD — lead gated 2026-09-18 (`Builds/compile-gate-1868.log` 11:12 `COMPILE_GATE_OK`, `Builds/data-regression-1868.log` 11:17 `REGRESSION_OK 580/580`), committed `424e4246a` 11:18. NOT on the Seeker: the installed build `2026.09.18.374427` has `lastUpdateTime=2026-09-17 19:34:21` (adb dumpsys), which predates the commit. Next APK carries it; owner felt-verify closes. PRIOR STATUS: READY FOR LEAD REVIEW — 2026-09-18 implemented by SME lane: tower VFX style fix
