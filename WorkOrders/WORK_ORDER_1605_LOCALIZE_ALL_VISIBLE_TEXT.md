...
- exact reconciliation of the Unity English table to 335 canonical keys, including
  stale-key removal and Smart String metadata;
...
Evidence: the latest canonical import reports 335 current entries and zero stale entries removed;
the focused suite reports `LOCALIZATION_REGRESSION_OK 5/5 suites`, and the full registered
suite reports `REGRESSION_OK 462/462 suites`. CompileGate's project
...
The manifest currently contains 5,977 report rows (335 keyed entries, 86 keyed wrapper
calls, 5,540 candidates, and 16 raster reviews). Its exact 2,994-fingerprint literal-debt
